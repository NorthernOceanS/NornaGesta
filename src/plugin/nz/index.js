import { systemInstance as system, BuildInstruction, Description, Usage } from 'norma-core';
import { setParameterForSummon_City2, Summon_City2, injectSetBlock, injectPlog } from './诺玛快速建造.js';

system.registerCanonicalGenerator({
    description: new Description("SCG", new Usage([], [], [], [
        { "viewtype": "edittext", "text": "长度", "key": "city_length" },
        { "viewtype": "edittext", "text": "宽度", "key": "city_width" },
        {
            "viewtype": "button", "text": "随机颜色配置", "key": "random_style", data: [
                {
                    value: 0,
                    text: "随机化颜色(关)"
                },
        ...
        {
            "viewtype": "button", "text": "地基配置", "key": "diji", data: [
                { value: 0, text: "地基方块(磨制安山岩)" },
                { value: 1, text: "地基方块(磨制闪长岩)" },
                { value: 2, text: "地基方块(双层平滑石台阶)" },
                { value: 3, text: "地基方块(石砖)" }
            ]
        },
        {
            "viewtype": "button",
    ...
    criteria: { positionArrayLength: 1, blockTypeArrayLength: 0, directionArrayLength: 1 },
    option: {
        city_length: 183,
        city_width: 120,
        random_style: 0,
    ...
    method: {
        generate: function (e) {
            ...
            let { city_length, city_width, random_style, diji, left, right } = e.state

            let position = e.state.positions[0], direction = e.state.directions[0]
            let deg = direction.x < 0 ? direction.x + 360 : direction.x
            e.runtime.logger.log("verbose", "NZ IS JULAO")
            e.runtime.logger.logObject("verbose", position)
            injectSetBlock(function (x, y, z, blockIdentifier, tiledata) {
                buildInstructions.push(new BuildInstruction("setblockWithTiledata", { x, y: y + position.coordinate.y/*Add base y*/, z, blockIdentifier, tiledata }))
            })
            injectPlog({ write: function (m) { e.runtime.logger.logObject("verbose", m) }, flush: () => { } })
            setParameterForSummon_City2({ city_length, city_width, random_style, diji, left, right })
            Summon_City2(position.coordinate.x, position.coordinate.y, position.coordinate.z, direction.x, deg)
